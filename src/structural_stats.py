"""Phase 3: structural statistics, benchmarked against a degree-preserving
...
(no model construction happens in this file):
  ...
  #3 our Jaccard (full, uncurated)
                              (data/processed/projection_full_uncurated.graphml)

Main analysis (default) runs #1 and #2 with N_NULL_RUNS=100 realizations.
--selection-effect-check runs #2 vs #3 side by side, to check how much #2's
>=5-shared-comics curation inflates clustering/z-scores versus the full
population (fewer runs by default since #3 is ~7x denser and much slower).
...
random node sample (documented below) to keep large ensembles tractable;
...
def run_selection_effect_check(n_runs=30):
    """Does curating to well-connected characters (>=5 shared comics, model #2)
    inflate clustering/z-scores versus the full, uncurated model #3? Loads
    both pre-built graphs (from build_projection.py) and compares their
    null-model stats side by side."""
    G_full = nx.read_graphml(PROC_DIR / "projection_full_uncurated.graphml")
    print(f"Loaded model #3 (full, uncurated): nodes={G_full.number_of_nodes()}, edges={G_full.number_of_edges()}")

    print(f"\nRunning {n_runs}-realization null ensemble on FULL network (this will take a while)...")
    rs_full = real_stats(G_full, exact_apl=False)
    print(f"Real stats (full): {rs_full}")
    null_full = null_ensemble_stats(G_full, n_runs=n_runs)

    G_curated = nx.read_graphml(PROC_DIR / "projection_jaccard.graphml")
    rs_curated = real_stats(G_curated, exact_apl=False)
    null_curated = null_ensemble_stats(G_curated, n_runs=n_runs)

    print(f"\n{'='*70}")
    print("FULL (6,439 chars, no curation) vs CURATED (2,396 chars, >=5 shared comics)")
    print(f"{'='*70}")
    for key in ["avg_clustering", "transitivity", "avg_shortest_path"]:
        rf, rc = rs_full[key], rs_curated[key]
        mf, sf, zf = zscore(rf, null_full[key])
        mc, sc, zc = zscore(rc, null_curated[key])
        print(f"\n{key}:")
        print(f"  FULL:    real={rf:.4f}  null_mean={mf:.4f}  z={zf:.2f}")
        print(f"  CURATED: real={rc:.4f}  null_mean={mc:.4f}  z={zc:.2f}")

    fit_degree_distribution(giant_component(G_full), "model #3 (full, uncurated)")
    fit_degree_distribution(giant_component(G_curated), "model #2 (curated)")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--selection-effect-check", action="store_true",
                         help="Run the full-vs-curated selection-effect comparison instead of the main analysis")
    parser.add_argument("--n-runs", type=int, default=30)
    args = parser.parse_args()

    if args.selection_effect_check:
        run_selection_effect_check(n_runs=args.n_runs)
    else:
        run_main_analysis()
